...
export default function ProjectCard({ id, title, description, image, technologies, githubUrl, liveUrl }) {
  ...
  return (
    <motion.article
      variants={fadeInUp}
      className="group flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-bg-surface transition-colors duration-300 hover:border-violet-400/50"
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="font-display text-base font-semibold text-ink-100">
          <Link to={`/projects/${id}`} className="hover:text-violet-300">{title}</Link>
        </h3>
        <p className="mt-2 flex-1 text-sm leading-relaxed text-ink-400">{description}</p>

        <div className="mt-4 flex flex-wrap gap-2">
          {visibleTech.map((tech) => <TechBadge key={tech} name={tech} />)}
          {extraCount > 0 && (
            <span className="rounded-full border border-white/10 px-3 py-1 text-xs text-ink-400">+{extraCount}</span>
          )}
        </div>
